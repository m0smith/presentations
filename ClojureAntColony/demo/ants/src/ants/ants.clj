...
; this is the same as 
; (mapv vec (take dim (partition dim (map ref (repeat (struct cell 0 0))))))

(def world
  (->> (repeat (struct cell 0 0)) 
       (map ref)
       (partition dim) 
       (take dim) 
       (mapv vec)))
...
(defn food-ahead? [ahead]
  (and (pos? (:food @ahead)) 
       (not (:home @ahead)) 
       (not (:ant @ahead))))

(defn home-ahead? [ahead]
  (and (:home @ahead) (not (:ant @ahead))))

(defn rank-foraging-places [places]
  (merge-with + 
              (rank-by (comp :food deref) places)
              (rank-by (comp :pher deref) places)))

(defn rank-home-places 
  "Return a map of places to an integer"
  [places]
  (merge-with + 
              (rank-by (comp #(if (:home %) 1 0) deref) places)
              (rank-by (comp :pher deref) places)))
  
(defn relative-place [loc ant offset]
  (place (delta-loc loc (+ offset (:dir ant)))))

(defn next-places [loc ant]
  (vec (map #(relative-place loc ant %) [0 -1 1])))

(defn wander [ranking-fn [ahead ahead-left ahead-right :as places] loc]
  (let [ranks (ranking-fn places)]

    (([move #(turn % -1) #(turn % 1)]
      (weighted-rand [(if (:ant @ahead) 0 (ranks ahead)) 
...
(defn behave
  ...
  (let [p (place loc)
    ...
    (dosync
     ...
     (if (:food ant)
       ;going home
       (cond (:home @p) (-> loc drop-food (turn 4))
             (home-ahead? ahead) (move loc)
         :else
           (wander rank-home-places places loc))


       ;foraging
       (cond (found-food? p) (-> loc take-food (turn 4))
             (food-ahead? ahead) (move loc)
        :else
